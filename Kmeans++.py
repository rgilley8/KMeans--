...
def initialize_centroids_plus(X, k, rng):
    """Choose k starting centroids by picking k rows of X at random,
    WITHOUT replacement."""
    # Randomly select the first index
    first_idx = rng.choice(X.shape[0], replace=False)
    # Create a list containing the centroids and initialize it with a centroid
    # at the first index you selected
    centroids = [X[first_idx]]

    # Run a for loop to select the rest of the k centroids one at a time
    for _ in range(1, k):
        # Compute the squared distances from the existing centroids
        squared_distances = np.sum((X[:, None, :] - np.array(centroids)[None, :, :]) ** 2, axis=2)
        # Find the minimum squared distance from each point to the nearest centroid
        min_squared_distances = np.min(squared_distances, axis=1)
        # Set probabilities proportional to the squared distances
        probabilities = min_squared_distances / np.sum(min_squared_distances)
        # Randomly select the next centroid index based on the probabilities
